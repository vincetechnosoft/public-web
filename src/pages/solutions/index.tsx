...
import SolutionData, { data } from "data/solutions";
...
const Solutions: NextPage = () => {
  return (
    <HomeLayout>
      <MentionSolutionKit
        id="distributors-solution"
        title="Distributor's Solution"
        src={[SolutionData.distributor, SolutionData.distributorClient]}
      />
      <MentionSolutionKit
        base="base2-page"
        id="milk-management"
        title="Milk Management"
        src={[SolutionData.milkManagement]}
      />
      <SoftwareInfo base="base1-page" />
      <ContactUs invert />
    </HomeLayout>
  );
};
...
interface MentionSolutionKitProps {
  id: string;
  title: string;
  src: data[];
  base?: "base1-page" | "base2-page";
}

function MentionSolutionKit(props: MentionSolutionKitProps) {
  return (
    <>
      <div id={props.id} />
      <div className={`min-h-[100vh] py-10 ${props.base ?? "base1-page"}`}>
        <h1 className="mx-5 mb-10 flex items-center text-4xl underline-offset-2 hover:underline md:mx-24">
          <LinkIcon className="mr-3" />
          <Link href={{ pathname: "/solutions", hash: props.id }}>
            <a>{props.title}</a>
          </Link>
        </h1>
        <div className="mx-5 lg:mx-10">
          {props.src.map(
            ({ capabilities, description, iconPath, id, title }, i) => (
                ...
                <div id={id} className="clickable-card relative mb-10 sm:flex">
                  ...
                  <div className="ml-5 hidden p-5 md:block">
                    ...
                    <ul className="ml-10 mt-5 list-disc">
                      {capabilities.slice(0, 3).map((x, i) => (
...
function SoftwareInfo({ base }: { base?: "base1-page" | "base2-page" }) {
  const [show, setShow] = useState(false);
  return (
    <div className={`${base ?? "base2-page"} py-10`}>
